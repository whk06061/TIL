...
<br/>그룹 단어란 `1. 단어에 해당 알파벳이 한번만 등장 2. 두번 이상 등장한다면 연달아 나와야 한다.` <br/>이것을 코드로 어떻게 정리할 수 있을까 생각했다. 처음에 생각한 것은 1번 조건은 문자열을 리스트로 변환 후 세트로 변환하여 세트의 길이가 리스트의 길이와 같을 때를 검사하는 코드를 생각했다. 그러나 2번을 어떻게 검사할 수 있을지 감이 오지 않았다.
